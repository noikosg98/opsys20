...
// Settling up the system counter, binding this with yield
int yield_count = 0;
...
rlnode SCHED[PLISTS]; /* The scheduler queue */
...
{
	int plist = tcb->priority;
	/* Insert at the end of the scheduling list */
	rlist_push_back(&SCHED[plist], &tcb->sched_node);
...
static TCB* sched_queue_select(TCB* current)
{
	...
	//Managing the boost condition
	if(yield_count == 50) {
		for(int i = 0; i < PLISTS; i++) {
		rlnode current_node = SCHED[i];
		TCB* current_tcb = current_node.tcb;
		while(current_tcb != NULL) {
			current_tcb->priority -= 1;
			rlist_pop_front(&current_node);
		}
		current_node = *current_node.next;
		current_tcb = current_node.tcb;
		}
	}
...
void yield(enum SCHED_CAUSE cause)
{
	...
	TCB* current = cur_thread(); /* Make a local copy of current process, for speed */
	...
	/* Switch contexts */
	if (current != next) {
		cur_thread()->ptcb->tcb = next;
		cpu_swap_context(&current->context, &next->context);
		//Edw stamataei to task-1
	}
...
void gain(int preempt)
{
	...
	TCB* current = cur_thread()->ptcb->tcb;
...
void run_scheduler()
{
	...
	/* Finished scheduling */
	assert(cur_thread() == &CURCORE.idle_thread);
